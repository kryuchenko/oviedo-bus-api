...
class CameraSelectionOptimizer {
    ...
    private static String decideSkippedCameraIdByHeuristic(CameraManagerCompat cameraManagerCompat, Integer num, List<String> list) throws CameraAccessExceptionCompat {
        if (num != null && list.contains(StdEntropyCoder.DEF_THREADS_NUM) && list.contains("1")) {
            if (num.intValue() == 1) {
                if (((Integer) cameraManagerCompat.getCameraCharacteristicsCompat(StdEntropyCoder.DEF_THREADS_NUM).get(CameraCharacteristics.LENS_FACING)).intValue() == 1) {
                    return "1";
                }
