...
public class DG11File extends DataGroup {
                ...
                if (iIntValue != 24386) {
                    switch (iIntValue) {
                        ...
                        case TELEPHONE_TAG /* 24338 */:
                            tLVOutputStream.writeTag(iIntValue);
                            tLVOutputStream.writeValue(this.telephone.trim().replace(' ', Typography.less).getBytes(Key.STRING_CHARSET_NAME));
                            break;
                        case PROFESSION_TAG /* 24339 */:
                            tLVOutputStream.writeTag(iIntValue);
                            tLVOutputStream.writeValue(this.profession.trim().replace(' ', Typography.less).getBytes(Key.STRING_CHARSET_NAME));
                            break;
                        case TITLE_TAG /* 24340 */:
                            tLVOutputStream.writeTag(iIntValue);
                            tLVOutputStream.writeValue(this.title.trim().replace(' ', Typography.less).getBytes(Key.STRING_CHARSET_NAME));
                            break;
                        case PERSONAL_SUMMARY_TAG /* 24341 */:
                            tLVOutputStream.writeTag(iIntValue);
                            tLVOutputStream.writeValue(this.personalSummary.trim().replace(' ', Typography.less).getBytes(Key.STRING_CHARSET_NAME));
                            break;
                        ...
                        case OTHER_VALID_TD_NUMBERS_TAG /* 24343 */:
                            ...
                            for (String str3 : this.otherValidTDNumbers) {
                                if (str3 != null) {
                                    ...
                                    tLVOutputStream.write(str3.trim().replace(' ', Typography.less).getBytes(Key.STRING_CHARSET_NAME));
                                }
                            }
                            tLVOutputStream.writeValueEnd();
                            break;
                        case CUSTODY_INFORMATION_TAG /* 24344 */:
                            tLVOutputStream.writeTag(iIntValue);
                            tLVOutputStream.writeValue(this.custodyInformation.trim().replace(' ', Typography.less).getBytes(Key.STRING_CHARSET_NAME));
                            break;
                        default:
    ...
    private void parseOtherValidTDNumbers(byte[] bArr) {
        ...
        try {
            strTrim = new String(bArr, Key.STRING_CHARSET_NAME);
        } catch (UnsupportedEncodingException e) {
            LOGGER.log(Level.WARNING, "Exception", (Throwable) e);
        }
